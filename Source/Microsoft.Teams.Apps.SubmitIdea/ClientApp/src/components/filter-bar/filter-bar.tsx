...
import { getLocalizedPostTypes, getLocalizedSortBy, generateColor } from "../../helpers/helper";
...
class FilterBar extends React.Component<IFilterBarProps, IFilterBarState> {
    ...
    constructor(props: IFilterBarProps) {
        ...
        const postTypes: Array<ICheckBoxItem> = this.props.categoryList.map((category: any, index: number) => {
            return {
                id: category.categoryId,
                key: index,
                checkboxLabel: <Flex vAlign="center">
                    <Status styles={{ backgroundColor: generateColor() }} />&nbsp;<Text content={category.categoryName} title={category.categoryName} />
                </Flex>,
    ...
    componentWillReceiveProps(nextProps: IFilterBarProps) {
        ...
        if (nextProps.categoryList !== this.props.categoryList) {
            this.setState({
                typeList: nextProps.categoryList.map((category: any, index: number) => {
                    return {
                        id: category.categoryId,
                        key: index,
                        checkboxLabel: <Flex vAlign="center">
    ...
    */
    onSortByStateChange = (selectedSortBy: string) => {
                            ...
                            <Flex.Item push>
                                <>
                                    <div className="filter-bar-item-container">
                                        <PopupMenuWrapper title={this.localize("category")} showSearchBar={false} selectedSortBy={this.state.selectedSortBy} checkboxes={this.state.typeList} onRadiogroupStateChange={this.onSortByStateChange} onCheckboxStateChange={this.onTypeCheckboxStateChange} />
