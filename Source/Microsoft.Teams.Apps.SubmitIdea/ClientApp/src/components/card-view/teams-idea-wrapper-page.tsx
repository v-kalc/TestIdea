...
import { getTeamDiscoverPosts, getUserVotes, getFilteredPosts, filterTitleAndTagsTeam } from "../../api/idea-api";
...
class TeamsIdeaWrapperPage extends React.Component<WithTranslation, ICardViewState> {
    ...
    */
    getFilteredDiscoverPosts = async (pageCount: number) => {
        let postTypes = this.selectedPostType.map((postType: ICheckBoxItem) => { return postType.id.toString() });
        ...
        if (response.status === 200 && response.data) {
            for (let i = 0; i < response.data.length; i++) {
                this.allPosts.map((post: IDiscoverPost) => {
                    if (post.ideaId === response.data[i].postId) {
                    ...
                    return (
                        <div className="container-div">
                            <div className="container-subdiv">
                                ...
                                <NoPostAddedPage showAddButton={false} botId="" onNewIdeaAdded={() => { }} />
                            </div>
                        </div>
                    )
                }
...
export default withTranslation()(TeamsIdeaWrapperPage)